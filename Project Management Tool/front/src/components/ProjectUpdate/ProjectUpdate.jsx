...
export default function ProjectUpdate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.patch(`/api/projects/${_id}`, updateProject);
      navigate(`/dashboard/project/${_id}`);
    } catch (err) {
      let error = err.response.data.message;
      if (error.startsWith("Project validation failed: ")) {
        error = error.slice("Project validation failed: ".length);
      }
      setError(error);
    }
  };
...
export const loader=async(arg)=>{
  const response = await axiosInstance.get(`/api/projects/${arg.params._id}`);
